...
const colorOptions = [
  { value: 'INFO', label: '정보(파란색)', className: 'bg-blue-600' },
  { value: 'WARNING', label: '경고(황색)', className: 'bg-amber-600' },
  { value: 'ERROR', label: '오류(빨간색)', className: 'bg-red-600' },
];
...
const ImportDebugPost = () => {
  ...
  const handleClick = useCallback(() => {
    openModal({
      title: t('import_debug_post', '디버그 게시물 가져오기'),
  ...
  return (
    <div
      ...
      onClick={handleClick}
    >
      {t('import_debug_post', '디버그 게시물 가져오기')}
    </div>
  );
};
