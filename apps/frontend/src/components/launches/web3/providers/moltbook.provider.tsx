...
export const MoltbookProvider: FC<Web3ProviderInterface> = (props) => {
  ...
  const register = async () => {
    if (!agentName.trim()) {
      toaster.show('에이전트 이름을 입력해 주세요.', 'warning');
      return;
    }
    ...
    try {
      const response = await fetch('/integrations/moltbook/register', {
        ...
        body: JSON.stringify({
          name: agentName.trim(),
          description: agentDescription.trim() || 'snaps social media scheduler',
        }),
      });
  ...
  const copyClaimUrl = useCallback(() => {
    copy(claimUrl);
    toaster.show('claim URL을 클립보드에 복사했습니다.', 'success');
  }, [claimUrl, toaster]);
