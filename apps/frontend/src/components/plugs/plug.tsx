    ...
    <>
      ...
      <CopilotTextarea
        ...
        autosuggestionsConfig={{
          textareaPurpose: `소셜 미디어 게시물을 작성하도록 도와주세요.`,
          chatApiConfigs: {},
        }}
      />
...
export const PlugPop: FC<{
  ...
  const submit: SubmitHandler<any> = useCallback(async (data) => {
    ...
    toaster.show('플러그가 업데이트되었습니다.', 'success');
    closeAll();
  }, []);
      ...
      <form onSubmit={form.handleSubmit(submit)}>
        <div className="relative mx-auto">
          ...
          <div className="mt-[20px]">
            <Button type="submit">{t('activate', '활성화')}</Button>
          </div>
        </div>
      </form>
    </FormProvider>
  ...
  return (
    ...
    >
      <div key={plug.title} className="p-[16px] h-full flex flex-col flex-1">
        ...
        <Button>{!data ? '플러그 설정' : '플러그 편집'}</Button>
      </div>
    </div>
  );
};
