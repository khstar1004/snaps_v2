...
function getHelpfulReasonForRegistrationFailure(
  httpCode: number,
  responseText: string
) {
  ...
  if (httpCode === 404) {
    return 'API 서버에 연결할 수 없습니다. NEXT_PUBLIC_BACKEND_URL 설정과 백엔드 실행 상태를 확인해 주세요.';
  }

  return (
    responseText ||
    `회원가입 처리 중 오류가 발생했습니다. 상태 코드: ${httpCode}`
  );
}
    ...
    await fetchData('/auth/register', {
      ...
      .then(async (response) => {
        ...
        if (response.status === 200) {
          ...
          form.setError('email', {
            message: getHelpfulReasonForRegistrationFailure(
              response.status,
              responseText
            ),
          });
      ...
      .catch((e) => {
        form.setError('email', {
          message: `회원가입 처리 중 오류가 발생했습니다. 브라우저 콘솔을 확인해 주세요. (${e.toString()})`,
        });
      });
  };
  return (
    <FormProvider {...form}>
      <form className="flex-1 flex" onSubmit={form.handleSubmit(onSubmit)}>
        <div className="flex flex-col flex-1">
          <div>
            <h1 className="text-[40px] font-[500] tracking-[0] text-start cursor-pointer">
              {t('sign_up', '회원가입')}
            </h1>
          </div>
          <div className="text-[14px] mt-[32px] mb-[12px]">
            {t('continue_with', '다음으로 계속')}
          </div>
          <div className="flex flex-col">
            ...
            <div className="flex flex-col gap-[12px]">
              <div className="text-textColor">
                ...
                <Input
                  label={t('label_company', '회사')}
                  translationKey="label_company"
                  {...form.register('company')}
                  autoComplete="off"
                  type="text"
                  placeholder={t('label_company', '회사')}
                />
              </div>
              ...
              <div className="text-center mt-6">
                ...
                <p className="mt-4 text-sm">
                  {t('already_have_an_account', '이미 계정이 있으신가요?')}
                  &nbsp;
                  <Link
                    href="/auth/login"
