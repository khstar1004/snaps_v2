...
export const DeveloperComponent: FC = () => {
  ...
  const createApp = useCallback(async () => {
    if (!name || !redirectUrl) {
      toaster.show('앱 이름과 리디렉션 URL을 입력해 주세요.', 'warning');
      return;
    }
    try {
      const result = await (
      ...
      if (result.clientSecret) {
        ...
        toaster.show(
          '앱을 만들었습니다. 클라이언트 시크릿은 한 번만 표시되므로 지금 복사해 주세요.',
          'success'
        );
      }
      setCreating(false);
      ...
      toaster.show('앱을 만들지 못했습니다.', 'warning');
    }
  ...
  const updateApp = useCallback(async () => {
    try {
      ...
      toaster.show('앱을 업데이트했습니다.', 'success');
      setEditing(false);
      mutate();
    } catch {
      toaster.show('앱 업데이트에 실패했습니다.', 'warning');
    }
  }, [name, description, redirectUrl, pictureId]);

  const rotateSecret = useCallback(async () => {
    const approved = await decision.open({
      title: '클라이언트 시크릿을 재발급할까요?',
      description:
        '새 클라이언트 시크릿이 생성되고 현재 시크릿은 즉시 만료됩니다. 이전 시크릿을 쓰는 연동은 다시 설정해야 합니다.',
      approveLabel: '재발급',
      cancelLabel: '취소',
    });
    if (!approved) return;
    try {
      ...
      if (result.clientSecret) {
        setPlaintextSecret(result.clientSecret);
        toaster.show(
          '시크릿을 재발급했습니다. 새 클라이언트 시크릿을 지금 복사해 주세요.',
          'success'
        );
        mutate();
      }
    } catch {
      toaster.show('시크릿 재발급에 실패했습니다.', 'warning');
    }
  }, [decision]);

  const deleteApp = useCallback(async () => {
    const approved = await decision.open({
      title: 'OAuth 앱을 삭제할까요?',
      description:
        'OAuth 앱을 삭제하고 모든 사용자 승인 권한을 해제합니다. 이 작업은 되돌릴 수 없습니다.',
      approveLabel: '삭제',
      cancelLabel: '취소',
    });
    if (!approved) return;
    try {
      ...
      toaster.show('OAuth 앱을 삭제했습니다.', 'success');
      setPlaintextSecret(null);
      mutate();
    } catch {
      toaster.show('OAuth 앱 삭제에 실패했습니다.', 'warning');
    }
  }, [decision]);
      ...
      <div className="flex flex-col gap-[40px]">
        <div className="text-[14px] text-textColor leading-[1.7]">
          {t(
            ...
            'Create an OAuth App to let other snaps users authorize your product to post on their behalf.'
          )}
          <br />
          {t(
        ...
        <div className="bg-newBgColorInner rounded-[12px] border border-newBorder overflow-hidden">
            ...
            <div>
              ...
              <div className="text-[13px] text-customColor18 mt-[2px]">
                {t(
                  ...
                  'Create an OAuth application to allow third-party integrations with snaps on behalf of your users.'
                )}
              </div>
            </div>
          ...
          <div className="p-[20px]">
            <button
              ...
              className="cursor-pointer px-[20px] h-[44px] bg-[#0ea5a8] hover:bg-[#0b8f95] transition-colors text-white rounded-[8px] text-[15px] font-[600]"
            >
              {t('create_oauth_app', 'OAuth 앱 만들기')}
            </button>
          </div>
        </div>
      </div>
          ...
          <div className="p-[20px] flex flex-col gap-[16px]">
            ...
            <div className="flex flex-col gap-[6px]">
              ...
              <textarea
                ...
                placeholder="앱의 역할을 설명해 주세요"
                maxLength={500}
              />
            </div>
            <div className="flex flex-col gap-[6px]">
              <label className="text-[13px] font-[600] text-customColor18">
                {t('profile_picture', '프로필 이미지')}
              </label>
              <div className="flex items-center gap-[12px]">
                ...
                >
                  {t('choose_image', '이미지 선택')}
                </button>
              </div>
            </div>
            <div className="flex flex-col gap-[6px]">
              <label className="text-[13px] font-[600] text-customColor18">
                {t('redirect_url', '리디렉션 URL')} *
              </label>
              <input
                className="bg-newBgColorInner border border-newBorder rounded-[8px] px-[16px] h-[44px] text-textColor outline-none"
                value={redirectUrl}
    ...
    <div className="flex flex-col gap-[40px]">
      ...
      <div className="bg-newBgColorInner rounded-[12px] border border-newBorder overflow-hidden">
        ...
        <div className="p-[20px] flex flex-col gap-[16px]">
          ...
          <div className="flex gap-[8px]">
            <CopyButton text={app.clientId} label={t('copy_id', 'ID 복사')} />
            {plaintextSecret && (
              <CopyButton
                text={plaintextSecret}
                label={t('copy_secret', '시크릿 복사')}
              />
            )}
            <button
              type="button"
