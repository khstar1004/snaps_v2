...
const ErrorDetailsModal: FC<{ row: ErrorRow }> = ({ row }) => {
  ...
  const copyAll = useCallback(() => {
    ...
    toaster.show('디버그 코드가 클립보드에 복사되었습니다.', 'success');
  ...
  return (
    <div className="rounded-[4px] border border-newTableBorder bg-newBgColorInner px-[16px] pb-[16px] relative w-full max-h-[80vh] overflow-auto">
      <div className="sticky top-0 bg-newBgColorInner py-[16px] flex items-center justify-between gap-[12px] z-10 border-b border-newTableBorder mb-[12px]">
        <div className="text-[16px] font-[600]">오류 상세</div>
        <div className="flex gap-[8px] items-center">
          <Button onClick={copyAll}>디버그 코드 복사</Button>
          <button
      ...
      <div className="grid grid-cols-2 gap-[12px] text-[13px] mb-[12px]">
        <div>
          <div className="opacity-60">플랫폼</div>
          <div>{row.platform}</div>
        </div>
        <div>
          <div className="opacity-60">발생 시간</div>
          <div>{new Date(row.createdAt).toLocaleString()}</div>
        </div>
        <div>
          <div className="opacity-60">조직</div>
          <div>
        ...
        <div>
          <div className="opacity-60">사용자</div>
        ...
        <div className="col-span-2">
          <div className="opacity-60">게시물 ID</div>
          <div>{row.postId}</div>
        </div>
      </div>
  ...
  return (
    <div className="flex flex-col gap-[16px] text-textColor">
      <div className="flex items-center justify-between">
        <div className="text-[20px] font-[600]">오류</div>
      ...
      <div className="flex flex-wrap gap-[12px] items-end bg-newBgColorInner border border-newTableBorder rounded-[8px] p-[12px]">
        <div className="flex flex-col gap-[6px]">
          <div className="text-[12px] opacity-70">플랫폼</div>
          <select
            value={platform}
            onChange={(e) => {
              setPage(0);
        ...
        <div className="flex flex-col gap-[6px]">
          <div className="text-[12px] opacity-70">이메일 포함</div>
          <div className="flex gap-[8px]">
            ...
            />
            <Button onClick={onApplyEmail}>적용</Button>
          </div>
        </div>
        ...
        <div className="flex flex-col gap-[6px]">
          <div className="text-[12px] opacity-70">페이지당 개수</div>
          <select
            value={limit}
            onChange={(e) => {
              setPage(0);
      ...
      {isLoading ? (
        ...
        <div className="text-red-400">오류 목록을 불러오지 못했습니다.</div>
      ) : !data || data.items.length === 0 ? (
        <div className="opacity-70">오류가 없습니다.</div>
      ) : (
            ...
            return (
              ...
              >
                ...
                <div className="flex gap-[8px] justify-end">
                  ...
                  <Button onClick={() => copyRow(row)}>복사</Button>
                </div>
              </div>
            );
          })}
